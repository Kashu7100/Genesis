...
def main():
    ...
    scene = gs.Scene(
        ...
        viewer_options=gs.options.ViewerOptions(camera_pos=(0.35, -0.35, 0.25), camera_lookat=(0.0, 0.0, 0.08)),
    ...
    for side, sign in (("L", -1.0), ("R", 1.0)):
        ...
        finger = scene.add_entity(
            gs.morphs.Mesh(
                file=os.path.join(args.asset_dir, "assets", FINGER_MESH.format(side=side)),
                pos=tuple(link_pos),
                euler=(180.0, 0.0, 0.0),
            ),
    ...
    if args.record:
        cam = scene.add_camera(res=(640, 360), pos=(0.35, -0.35, 0.25), lookat=(0.0, 0.0, 0.08))
    scene.build()
    ...
    for i_step in range(args.n_steps):
        if 60 <= i_step < 120:
            ...
            gripper.control_dofs_position(np.array([0.033 * (i_step - 59) / 60.0]), np.array([0]))
        scene.step()
        if i_step % 60 == 0:
            ...
            print(
                f"step {i_step:4d}: box z={box_z:.4f} finger gap={tip_gap:+.4f} "
